...
data.frame(magnitud_por_magtype, n = Inf)
...
v_cramer_magtype_grupo_magsource <- sqrt(
  as.numeric(prueba_chi_magtype_grupo_magsource$statistic) /
    (
      sum(tabla_magtype_grupo_magsource) *
        (min(dim(tabla_magtype_grupo_magsource)) - 1)
    )
)
...
asociacion_magtype_grupo_magsource
...
asociacion_magtype_profundidad
