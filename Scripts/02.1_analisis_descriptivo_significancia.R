...
correlaciones_sig <- tibble::tibble(
  variable = c("mag", "depth", "nst_imp", "rms_imp"),
  observaciones = c(
    sum(complete.cases(sismos$sig, sismos$mag)),
    sum(complete.cases(sismos$sig, sismos$depth)),
    sum(complete.cases(sismos$sig, sismos$nst_imp)),
    sum(complete.cases(sismos$sig, sismos$rms_imp))
  ),
  correlacion_pearson = c(
    cor(sismos$sig, sismos$mag, method = "pearson", use = "complete.obs"),
    cor(sismos$sig, sismos$depth, method = "pearson", use = "complete.obs"),
    cor(sismos$sig, sismos$nst_imp, method = "pearson", use = "complete.obs"),
    cor(sismos$sig, sismos$rms_imp, method = "pearson", use = "complete.obs")
  ),
  correlacion_spearman = c(
    cor(sismos$sig, sismos$mag, method = "spearman", use = "complete.obs"),
    cor(sismos$sig, sismos$depth, method = "spearman", use = "complete.obs"),
    cor(sismos$sig, sismos$nst_imp, method = "spearman", use = "complete.obs"),
    cor(sismos$sig, sismos$rms_imp, method = "spearman", use = "complete.obs")
  ),
  valor_p_spearman = c(
    suppressWarnings(cor.test(sismos$sig, sismos$mag, method = "spearman")$p.value),
    suppressWarnings(cor.test(sismos$sig, sismos$depth, method = "spearman")$p.value),
    suppressWarnings(cor.test(sismos$sig, sismos$nst_imp, method = "spearman")$p.value),
    suppressWarnings(cor.test(sismos$sig, sismos$rms_imp, method = "spearman")$p.value)
  )
) %>%
