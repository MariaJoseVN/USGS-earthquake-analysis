...
#02_analisis_descriptivo.R, 02.1_analisis_descriptivo_significancia.R,
#02.2_analisis_descriptivo_categoricas.R y 05_analisis_asociatividad.R.
#Requiere que Codigo.R ya haya preparado sismos (con magnitud_cat, profundidad_cat,
#magType_grupo, magSource y sig) y que 01_tratamiento_NAs.R haya creado nst_imp y rms_imp.
...
count(sismos, zona, name = "numero_eventos")
...
comparabilidad_zona <- sismos %>%
  ...
  summarise(
    ...
    magType_dominante = names(which.max(table(magType_grupo, useNA = "no"))),
    porcentaje_magType_dominante = max(table(magType_grupo, useNA = "no")) /
      sum(table(magType_grupo, useNA = "no")) * 100,
    magSource_dominante = names(which.max(table(magSource, useNA = "no"))),
    porcentaje_magSource_dominante = max(table(magSource, useNA = "no")) /
      sum(table(magSource, useNA = "no")) * 100,
    .groups = "drop"
  ) %>%
  tidyr::complete(zona = zonas_estudio) %>%
  mutate(
