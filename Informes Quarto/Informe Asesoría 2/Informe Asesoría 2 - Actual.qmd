...
## Síntesis del Informe Asesoría 1 y paso a la etapa inferencial {#sec-sintesis-informe-1}

El \asesoriaExternalLink{https://mariajosevn.github.io/USGS-earthquake-analysis/Informes\%20Quarto/Informe\%20Asesor\%C3\%ADa\%201/Informe\%20Asesor\%C3\%ADa\%201\%20-\%20Actual.pdf}{Informe Asesoría 1} caracterizó 1.186 terremotos de magnitud $M \geq 6{,}5$ registrados por la United States Geological Survey (USGS) entre 2000 y 2025. Los principales hallazgos son la fuerte concentración espacial de la ocurrencia en el Cinturón de Fuego, la similitud general de las magnitudes medias entre zonas, la mayor diferenciación observada en profundidad y una serie temporal con fluctuaciones, pero sin una tendencia uniforme para todo el período.

Este informe toma esos patrones descriptivos como punto de partida y los reformula como preguntas inferenciales. Primero se revisa la comparabilidad del registro; luego, en frecuencia, se evalúa si las tasas de ocurrencia difieren formalmente entre zonas y si esa diferencia se mantiene al considerar la superficie. En magnitud y profundidad, se contrastan las distribuciones completas, no solo sus promedios, y se retoman las categorías de magnitud para examinar los eventos fuertes y extremos.

La dimensión temporal se aborda para verificar si las fluctuaciones observadas en el catálogo corresponden a cambios sostenidos en la ocurrencia o a variación propia del período analizado. Finalmente, la comparación entre zonas se extiende a un problema de clasificación: evaluar si la magnitud y la profundidad, consideradas en conjunto, aportan información para identificar la zona sísmica del evento.
...
\item \textbf{OE1. Verificar} la comparabilidad del registro entre zonas y períodos, mediante variables de reporte y ajuste de localización, para identificar posibles diferencias asociadas al procedimiento de medición.

\item \textbf{OE2. Comparar} la frecuencia de eventos entre zonas mediante tasas anuales, densidades por superficie y modelos de conteo, cuantificando razones de tasa e intervalos de confianza.

\item \textbf{OE3. Contrastar} las distribuciones de magnitud y profundidad entre zonas, evaluando los supuestos del análisis multivariante y aplicando la ruta no paramétrica correspondiente.

\item \textbf{OE4. Analizar} la estructura temporal de la ocurrencia mediante pruebas de estacionariedad, independencia serial, comparación de tasas por período y mes, y ajuste de tiempos entre eventos.

\item \textbf{OE5. Examinar} la composición de eventos fuertes, mayores y grandes o extremos por zona, mediante tablas de contingencia y modelos logísticos para el umbral $M \geq 7{,}0$.

\item \textbf{OE6. Evaluar} si la magnitud y la profundidad permiten clasificar la zona sísmica de un evento, mediante análisis de correspondencia, regresión logística multinomial y métricas de desempeño por clase.

\item \textbf{OE7. Verificar} la robustez de las conclusiones ante cambios en el umbral de magnitud y en la definición de las zonas de comparación.
...
Ante separación, la estimación de máxima verosimilitud diverge y se reemplaza por la corrección de reducción de sesgo [@kosmidis_firth_2011], según se declara en la clasificación de zonas.
