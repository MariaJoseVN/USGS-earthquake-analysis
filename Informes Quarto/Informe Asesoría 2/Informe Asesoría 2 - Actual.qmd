...
Las asociaciones categóricas se evaluaron mediante chi-cuadrado de independencia \asesoriaStyledRef{(véase \hyperref[sec-anexo-chi-esperadas]{anexo del contraste chi-cuadrado y frecuencias esperadas})}. Como algunas tablas tenían frecuencias esperadas bajas, especialmente en zonas con pocos eventos, descartamos fusionar categorías relevantes y usamos valores p obtenidos con 10.000 simulaciones de Monte Carlo [@hope_1968; @patefield_1981]. Esta decisión permite mantener la estructura original de reporte y evita depender de una aproximación chi-cuadrado poco estable en celdas escasas. La precisión de la simulación se presenta fuera del cuerpo principal. \asesoriaStyledRef{(véase \hyperref[sec-anexo-montecarlo]{anexo de precisión de la simulación Monte Carlo})}.
...
Como las frecuencias esperadas incluían celdas bajo 5 en las zonas de menor tamaño \asesoriaStyledRef{(véase \hyperref[sec-anexo-chi-esperadas]{anexo del contraste chi-cuadrado y frecuencias esperadas})}, el valor p del chi-cuadrado se obtuvo con 10.000 simulaciones de Monte Carlo, el mismo criterio definido para la comparabilidad de los procedimientos de reporte [@hope_1968; @patefield_1981]. El grado de asociación se midió con la V de Cramér corregida por sesgo, que descuenta la asociación aparente producida por la estructura muestral [@bergsma_2013]. \asesoriaStyledRef{(véase \hyperref[sec-anexo-montecarlo]{anexo de precisión de la simulación Monte Carlo})}.

Para la regresión logística, las categorías Mayor y Grande o extremo se agruparon en un indicador binario que vale 1 cuando $M \geq 7{,}0$ y 0 en caso contrario. El modelo estima la probabilidad de superar ese umbral en función de la zona y la profundidad consideradas simultáneamente, con el Cinturón de Fuego como referencia por ser la zona de mayor tamaño muestral [@mccullagh_nelder_1989] \asesoriaStyledRef{(véase \hyperref[sec-anexo-logistica]{anexo del modelo de regresión logística})}. Incluir ambas variables en un mismo modelo pone a prueba una posibilidad que los análisis por separado no cubren: que la profundidad, con poca asociación cuando se examina sola, aporte información una vez considerada la zona. La significación de cada variable se evaluó mediante razón de verosimilitudes por bloque \asesoriaStyledRef{(véase \hyperref[sec-anexo-verosimilitud]{anexo de verosimilitud y estimación de los modelos})}, porque la zona aporta tres coeficientes y debe contrastarse como una sola variable:
...
Las casillas en cero (el Cinturón Alpino-Himalayo y la Dorsal sin eventos profundos, y la Dorsal tampoco con intermedios) anticipan la separación que más adelante obliga a corregir el modelo multinomial. La `Tabla~\ref{tbl-clasificacion-inercia}`{=latex} resume la inercia de ambas tablas.
...
El modelo multinomial cuantifica ese contraste de manera formal \asesoriaStyledRef{(véase \hyperref[sec-anexo-multinomial]{anexo del modelo de regresión logística multinomial})}. La `Tabla~\ref{tbl-clasificacion-lr}`{=latex} presenta la razón de verosimilitudes por variable, calculada sobre el ajuste de máxima verosimilitud; la corrección de sesgo se aplica solo a los coeficientes reportados más adelante.
...
Para no limitar la evaluación a esa decisión final, se revisaron las curvas ROC uno contra el resto \asesoriaStyledRef{(véase \hyperref[sec-anexo-roc]{anexo de curvas ROC y área bajo la curva})}. Esta herramienta transforma el problema multinomial en cuatro comparaciones binarias, cada zona frente a todas las demás. Técnicamente, el área bajo la curva ROC (AUC) mide si las probabilidades ajustadas ordenan correctamente los eventos de una zona por encima de los que no pertenecen a ella. Un valor cercano a 0,5 indica discriminación similar al azar, mientras que valores más altos indican mejor separación. En términos no técnicos, la matriz de confusión pregunta "¿qué etiqueta final asignó el modelo?", mientras que la curva ROC pregunta "¿el modelo al menos elevó la probabilidad de la zona correcta, aunque no haya sido la etiqueta ganadora?". La `Figura~\ref{fig-clasificacion-roc}`{=latex} muestra esta evaluación complementaria.
...
La `Tabla~\ref{tbl-clasificacion-aic}`{=latex} compara los modelos anidados. La columna $\Delta$AIC expresa la diferencia respecto del modelo de menor AIC, y la columna $LR$ mide el aporte de agregar la variable faltante para llegar al modelo conjunto.
...
El mismo estadístico alimenta las dos medidas descriptivas del informe: dividido por $n$ produce $\phi^2$, la base de la V de Cramér corregida \asesoriaStyledRef{(véase \hyperref[sec-anexo-cramer]{anexo de corrección por sesgo de la V de Cramér})} y de la inercia total del análisis de correspondencia \asesoriaStyledRef{(véase \hyperref[sec-anexo-correspondencia]{anexo de análisis de correspondencia})}.
...
donde $n$ es el número de observaciones, $r$ y $c$ corresponden al número de filas y columnas de la tabla y $\chi^2$ es el estadístico del contraste de independencia \asesoriaStyledRef{(véase \hyperref[sec-anexo-chi-esperadas]{anexo del contraste chi-cuadrado y frecuencias esperadas})}. Para reducir su sesgo positivo se aplicó la corrección propuesta en la literatura [@bergsma_2013]:
...
Un valor de $D$ mayor indica una discrepancia más marcada, pero su interpretación depende del tamaño muestral. Además, el valor p convencional de Kolmogorov-Smirnov supone que la distribución de referencia está completamente especificada. En este análisis $\lambda$ se estima con los mismos tiempos que se contrastan, por lo que el valor p se obtuvo mediante bootstrap paramétrico [@durbin_1973] \asesoriaStyledRef{(véase \hyperref[sec-anexo-bootstrap-parametrico]{anexo de bootstrap paramétrico para pruebas de ajuste})}.
...
La V de Cramér y epsilon cuadrado se mueven entre 0 (sin asociación) y 1; delta de Cliff, entre $-1$ y 1, con 0 indicando superposición total entre las dos distribuciones comparadas; las razones de tasa y de odds comparan cada zona con la referencia, con el 1 como valor de igualdad. La estructura temporal no requirió una medida de asociación: sus resultados se expresan directamente como tasas anuales por período. El desarrollo de cada medida se encuentra en su anexo \asesoriaStyledRef{(véanse los anexos de \hyperref[sec-anexo-cramer]{corrección por sesgo de la V de Cramér}, \hyperref[sec-anexo-epsilon]{cálculo de epsilon cuadrado} y \hyperref[sec-anexo-or]{razón de odds})}; delta de Cliff se define en la especificación de dócimas y modelos \asesoriaStyledRef{(véase \hyperref[sec-anexo-especificacion]{anexo de especificación de dócimas y modelos})}. Ninguna conclusión del informe se establece con el valor p aislado: la decisión combina el valor p, el tamaño de efecto y su intervalo de confianza.
...
Ante separación, la estimación de máxima verosimilitud diverge y se reemplaza por la corrección de reducción de sesgo [@kosmidis_firth_2011], según se declara en la clasificación de zonas \asesoriaStyledRef{(véase \hyperref[sec-anexo-separacion]{anexo de separación y reducción de sesgo})}.
...
por lo que se conservó la especificación continua, más simple y sin pérdida de información por categorización. Se usó AIC y no una prueba de significación porque las dos representaciones no son modelos anidados en el sentido requerido por la razón de verosimilitudes.
...
La medida de partida es la inercia total, que cuantifica la asociación global de la tabla. Equivale al estadístico chi-cuadrado de independencia \asesoriaStyledRef{(véase \hyperref[sec-anexo-chi-esperadas]{anexo del contraste chi-cuadrado y frecuencias esperadas})} dividido por el número de observaciones:

$$
\text{inercia total}=\frac{\chi^2}{n}=\phi^2,
$$

donde $n$ es el número de eventos. Es la misma cantidad $\phi^2$ que sustenta la V de Cramér \asesoriaStyledRef{(véase \hyperref[sec-anexo-cramer]{anexo de corrección por sesgo de la V de Cramér})}: una inercia cercana a cero indica perfiles casi idénticos entre zonas, y una inercia mayor, perfiles más distinguibles.
...
Este anexo presenta el modelo utilizado para clasificar la zona de un evento a partir de su magnitud y profundidad. Es la extensión de la regresión logística a una respuesta con más de dos categorías \asesoriaStyledRef{(véase \hyperref[sec-anexo-logistica]{anexo del modelo de regresión logística})}: en lugar de una probabilidad de sí o no, el modelo asigna a cada evento cuatro probabilidades, una por zona, que suman 1 [@venables_ripley_2002].
...
un coeficiente extremo cuyo error estándar crece junto con él, lo que invalida la prueba de Wald sobre ese término [@hauck_donner_1977] \asesoriaStyledRef{(véase \hyperref[sec-anexo-wald]{anexo del contraste de Wald})}.
...
el mismo valor de la regla trivial, que también reconoce solo a la zona mayoritaria. La lectura conjunta es directa: el modelo no aporta capacidad clasificatoria adicional sobre la regla que asigna todo al Cinturón de Fuego. Las curvas ROC complementan esta evaluación: examinan si las probabilidades ajustadas contienen señal discriminante aunque la regla de máxima probabilidad no la convierta en asignaciones distintas \asesoriaStyledRef{(véase \hyperref[sec-anexo-roc]{anexo de curvas ROC y área bajo la curva})}.
...
Este anexo presenta los resultados completos de las comparaciones por pares posteriores a Kruskal-Wallis. La justificación del ajuste por multiplicidad se desarrolla en su anexo específico \asesoriaStyledRef{(véase \hyperref[sec-anexo-ajuste-p]{anexo de ajuste de valores p por multiplicidad})}. Los valores p ajustados mediante Holm son los que se utilizan para la decisión inferencial.
...
Este anexo documenta los diagnósticos utilizados para decidir entre los modelos Poisson y binomial negativo en la comparación de frecuencia entre zonas. La formulación, justificación e interpretación del modelo binomial negativo se presentan en el anexo correspondiente \asesoriaStyledRef{(véase \hyperref[sec-anexo-binomial-negativa]{anexo del modelo binomial negativo para conteos con sobredispersión})}. Aquí se presentan únicamente las comprobaciones aplicadas al catálogo y sus resultados.
