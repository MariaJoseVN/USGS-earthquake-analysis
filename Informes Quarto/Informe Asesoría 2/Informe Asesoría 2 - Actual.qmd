...
La binomial negativa se usa cuando la respuesta es un conteo, pero la variabilidad observada supera la prevista por Poisson [@mccullagh_nelder_1989; @cameron_trivedi_1990]. En la comparación de frecuencia entre zonas, cada observación corresponde a una combinación zona-año y la respuesta es el número de terremotos registrados. Por eso el punto de partida natural es Poisson; sin embargo, ese modelo supone $\operatorname{Var}(Y_i)=\mu_i$, condición que no siempre se cumple en conteos sísmicos agregados.
