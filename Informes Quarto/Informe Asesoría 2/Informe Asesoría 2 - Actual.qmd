...
\newgeometry{top=1.25cm,bottom=1.25cm,left=2.00cm,right=2.00cm,footskip=0.65cm}
...
Antes de cualquier comparación sustantiva, esta sección pregunta si los procedimientos de reporte difieren según la zona o según el período. Su respuesta condiciona el resto del informe, porque una asociación fuerte entre el reporte y la zona obligaría a incorporar controles en los modelos posteriores, mientras que una asociación con el período restringiría las comparaciones temporales de magnitud.
...
Este análisis se centra en la comparación espacial y pregunta si la tasa de ocurrencia difiere entre las cuatro zonas, tanto por año como por unidad de superficie. Para responderla, los 26 años se agregan y cada zona queda resumida por una tasa única del período. Esa agregación es defendible solo si la tasa no cambió de forma relevante dentro del período, condición que se evalúa en la metodología de estructura temporal de la ocurrencia.
...
Mientras la comparación de frecuencia pregunta cuántos eventos registra cada zona, este análisis examina cómo son esos eventos y evalúa si las distribuciones de magnitud y profundidad difieren entre las cuatro zonas. El Informe Asesoría 1 mostró magnitudes medias casi idénticas (6,90; 6,92 y 6,91) salvo la Dorsal Meso-Atlántica (6,69), y una profundidad con composición claramente distinta por zona; esta sección evalúa si esas diferencias descriptivas son estadísticamente defendibles.
...
El análisis temporal formaliza el hallazgo descriptivo del \asesoriaExternalLink{https://mariajosevn.github.io/USGS-earthquake-analysis/Informes\%20Quarto/Informe\%20Asesor\%C3\%ADa\%201/Informe\%20Asesor\%C3\%ADa\%201\%20-\%20Actual.pdf}{Informe Asesoría 1}, según el cual la ocurrencia anual fluctúa entre años, pero no muestra una tendencia visual uniforme. Este análisis complementa la comparación de frecuencia entre zonas. En esa sección resumimos el período completo para contrastar zonas; aquí agregamos las zonas para contrastar el tiempo. Ambos análisis usan los mismos conteos del catálogo, pero responden preguntas distintas. La comparación de frecuencia responde si la tasa de ocurrencia difiere entre zonas; este análisis evalúa si esa tasa se mantiene estable durante 2000-2025. Para abordar esa segunda pregunta, revisamos si los conteos fluctúan alrededor de un nivel estable (estacionariedad), si un período con muchos eventos tiende a ser seguido por otro igual de alto (autocorrelación), si algún tramo del período o algún mes concentra sistemáticamente más eventos (cambio de nivel y estacionalidad) y si los tiempos entre eventos consecutivos son compatibles con una tasa constante (ajuste exponencial). En este análisis usamos dos series de conteos construidas a partir del mismo catálogo. La serie anual permite comparar con la descripción previa y con los períodos de análisis, mientras que la serie mensual entrega mayor número de observaciones para diagnosticar estabilidad temporal, autocorrelación y estacionalidad.
...
El análisis de eventos fuertes y extremos retoma las categorías de magnitud definidas en el Informe Asesoría 1, correspondientes a Fuerte ($6{,}5 \leq M < 7{,}0$), Mayor ($7{,}0 \leq M < 7{,}8$) y Grande o extremo ($M \geq 7{,}8$). La pregunta es distinta de la abordada en la comparación de frecuencia. Ya no interesa cuántos eventos registra cada zona, sino la composición de severidad dentro de cada una y si la probabilidad de que un evento alcance las categorías superiores cambia según la zona en que ocurrió. El análisis se organizó en cuatro componentes: una tabla de contingencia entre zona y categoría de magnitud, una regresión logística para la probabilidad de que un evento alcance $M \geq 7{,}0$, una comparación entre dos representaciones de la profundidad dentro de ese modelo y una descripción de los eventos con $M \geq 7{,}8$ mediante conteos y tasas anuales.
...
Los resultados se interpretan como inferencia sobre el catálogo definido, compuesto por eventos $M \geq 6{,}5$ registrados por USGS entre 2000 y 2025, bajo la zonificación adoptada. Ninguna dócima o modelo de este informe representa estimación causal, predicción operacional ni evaluación probabilística de amenaza sísmica.
...
El modelo Poisson es el punto de partida para conteos de ocurrencias en unidades de exposición comparables [@mccullagh_nelder_1989; @venables_ripley_2002]. En esta sección, cada observación es un año del catálogo y la respuesta es el número de eventos registrados. Por eso Poisson es más adecuado que una comparación normal de medias: los conteos son enteros, no negativos y su varianza depende del nivel medio de ocurrencia.
...
Las odds de un suceso son el cociente entre la probabilidad de que ocurra y la de que no ocurra, y la razón de odds compara dos grupos dividiendo sus odds [@bland_altman_2000]:
...
El contraste de Wald evalúa si un coeficiente difiere de cero a partir de su estimación y su error estándar, sin reajustar el modelo:
...
Este anexo documenta los diagnósticos de especificación de los modelos de conteo. Los aspectos de separación y ajuste del modelo multinomial se declaran en la metodología y se interpretan junto con sus resultados.
