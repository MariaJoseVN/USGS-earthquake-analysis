...
La metodología se organiza en seis análisis encadenados más un análisis de sensibilidad. El primero revisa la comparabilidad del registro: si los procedimientos de reporte difirieran entre zonas, las comparaciones posteriores mezclarían diferencias sísmicas con diferencias de medición. Establecida esa base, tres análisis abordan la primera componente de la pregunta estadística desde cortes complementarios: la frecuencia compara cuántos eventos registra cada zona, la comparación de magnitud y profundidad examina cómo son esos eventos, y la estructura temporal examina si la tasa se mantuvo estable durante el período, condición que respalda la agregación de años empleada por las anteriores. El quinto análisis cruza ambas miradas al examinar la composición de severidad dentro de cada zona, y el sexto invierte la pregunta para responder la segunda componente: si la magnitud y la profundidad, en conjunto, identifican la zona de un evento. El análisis de sensibilidad final verifica la robustez de las conclusiones ante el umbral de magnitud y la definición de las zonas.
...
La pregunta de esta sección es previa a cualquier comparación: ¿difieren los procedimientos de reporte según la zona o según el período? Su respuesta condiciona el resto del informe: una asociación fuerte entre el reporte y la zona obligaría a incorporar controles en los modelos posteriores, mientras que una asociación con el período restringiría las comparaciones temporales de magnitud.
...
Mientras la comparación de frecuencia pregunta cuántos eventos registra cada zona, este análisis pregunta cómo son esos eventos: ¿difieren las distribuciones de magnitud y profundidad entre las cuatro zonas? El Informe Asesoría 1 mostró magnitudes medias casi idénticas (6,90; 6,92 y 6,91) salvo la Dorsal Meso-Atlántica (6,69), y una profundidad con composición claramente distinta por zona; esta sección evalúa si esas diferencias descriptivas son estadísticamente defendibles.
...
La capacidad clasificatoria se evaluó primero con la matriz de confusión bajo la regla de máxima probabilidad, que asigna cada evento a la zona con mayor probabilidad ajustada. A partir de ella se calcularon tres métricas: la exactitud global, comparada con la base trivial de clasificar siempre la zona mayoritaria (75,2 % de los eventos); la exactitud balanceada, promedio de las sensibilidades por zona, decisiva ante el desbalance porque pondera igual a las cuatro zonas; y la sensibilidad de cada zona, definida como la proporción de eventos de esa zona correctamente identificados.
