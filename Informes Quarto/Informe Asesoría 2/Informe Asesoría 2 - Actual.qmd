...
Ambos contrastes resultaron significativos ($p<0{,}001$), de modo que las cuatro zonas no comparten una misma tasa de ocurrencia ni una misma densidad por superficie. Establecida esa diferencia, la `\asesoriaTableRef{tbl-frecuencia-zonas}`{=latex} cuantifica su magnitud. La razón de tasa mide cuánto difiere cada zona del Cinturón de Fuego, tomado como referencia: un valor de 1 indica igual frecuencia y un valor menor que 1, una frecuencia menor. El intervalo de confianza del 95 % indica con qué precisión se conoce esa razón, es decir, el rango de valores compatibles con los datos.
