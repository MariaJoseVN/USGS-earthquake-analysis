...
Para la regresión logística, las categorías Mayor y Grande o extremo se agruparon en un indicador binario que vale 1 cuando $M \geq 7{,}0$ y 0 en caso contrario. El modelo estima la probabilidad de superar ese umbral en función de la zona y la profundidad consideradas simultáneamente, con el Cinturón de Fuego como referencia por ser la zona de mayor tamaño muestral [@mccullagh_nelder_1989] \asesoriaStyledRef{(véase \hyperref[sec-anexo-logistica]{anexo del modelo de regresión logística})}. Esta especificación conjunta responde la observación docente sobre el posible aporte de la profundidad dentro de un modelo múltiple: una variable con poca asociación marginal podría adquirir relevancia al combinarse con otra. La significación de cada variable se evaluó mediante razón de verosimilitudes por bloque \asesoriaStyledRef{(véase \hyperref[sec-anexo-verosimilitud]{anexo de verosimilitud y estimación de los modelos})}, porque la zona aporta tres coeficientes y debe contrastarse como una sola variable:
...
Los modelos del informe (conteos, logístico y multinomial) se estimaron por máxima verosimilitud [@fisher_1922]. Si $P(y_i \mid \beta)$ es la probabilidad que el modelo con coeficientes $\beta$ asigna a la observación $i$, la verosimilitud del catálogo completo es:

$$
L(\beta)=\prod_{i=1}^{n} P(y_i \mid \beta),
$$

y la estimación escoge el vector $\hat\beta$ que la maximiza: los coeficientes que hacen más probable lo efectivamente observado. A partir de ella se define la devianza, igual a $-2\log L$ salvo una constante; mientras menor, mejor el ajuste.
...
el valor reportado en la tabla de contrastes de esa sección. Ante separación, la estimación de máxima verosimilitud diverge y se reemplaza por la corrección de reducción de sesgo [@kosmidis_firth_2011], según se declara en la clasificación de zonas.
...
valor cercano al 0,247 del modelo logístico, que además controla por profundidad. Un valor de 1 indica odds iguales a la referencia; menor que 1, odds menores; mayor que 1, odds mayores. Se reporta esta medida porque es la que entrega la regresión logística (la exponencial de cada coeficiente) y porque no depende del nivel de base de la referencia. La razón de odds no equivale a una razón de probabilidades (riesgo relativo); ambas se aproximan solo cuando el suceso es poco frecuente, por lo que con 32,6 % de eventos sobre el umbral la lectura se mantiene en términos de odds.
...
en las comparaciones por pares \asesoriaStyledRef{(véase \hyperref[sec-anexo-wald]{anexo del contraste de Wald})}, y el de verosimilitud perfilada, que no supone simetría y es preferible con pocos eventos [@venables_ripley_2002]. La diferencia se aprecia en la razón de odds de la Dorsal Meso-Atlántica: 0,247 con intervalo [0,058; 0,712], asimétrico porque con 3 eventos sobre el umbral la incertidumbre no se reparte por igual a ambos lados.
...
Se utilizó donde se requieren varias comparaciones a partir de un mismo ajuste: los intervalos de las comparaciones por pares en frecuencia y los contrastes específicos de cada zona frente a la referencia en la regresión logística. No se usó para las decisiones globales por dos razones. Primero, evalúa un coeficiente a la vez, mientras que una variable como la zona aporta tres y debe contrastarse como bloque mediante razón de verosimilitudes \asesoriaStyledRef{(véase \hyperref[sec-anexo-verosimilitud]{anexo de verosimilitud y estimación de los modelos})}. Segundo, cerca de una separación el error estándar crece más rápido que el coeficiente y el estadístico pierde capacidad de detección justo cuando el efecto es más fuerte [@hauck_donner_1977].
...
donde $L$ es la verosimilitud maximizada y $k$ el número de parámetros. La penalización $2k$ protege contra el sobreajuste. El AIC no se interpreta en valor absoluto: solo compara modelos ajustados sobre los mismos datos y se prefiere el menor; una diferencia menor que 2 indica respaldo prácticamente equivalente y se resuelve por parsimonia [@burnham_anderson_2004]. Evaluado en la comparación entre las dos representaciones de la profundidad de los eventos fuertes:
