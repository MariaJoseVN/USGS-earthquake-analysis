...
Con la incorporacion de `magnitud_cat` en `Codigo.R`, tambien se agregaron conteos por categoria de magnitud:

- `conteo_anual_magnitud_cat`
- `conteo_decadal_magnitud_cat`

Estos objetos permiten describir la composicion temporal del catalogo entre eventos `Fuerte`, `Mayor` y `Grande o extremo`. Se decidio agregarlos para complementar los conteos por umbrales, no para reemplazarlos. Los umbrales acumulativos (`M >= 7.0`, `M >= 7.5`, `M >= 8.0`) siguen siendo utiles para preguntas especificas, mientras que `magnitud_cat` permite una lectura por grupos excluyentes de severidad.

No se incorporo un conteo mensual por categoria en esta etapa, porque habria exigido completar todas las combinaciones mes-categoria con ceros y podria sobrecargar el preinforme descriptivo. Para la primera lectura temporal, se priorizo la composicion anual y decadal.
...
Estos graficos usan barras apiladas para mostrar como se reparte el total de eventos entre `Fuerte`, `Mayor` y `Grande o extremo`. La variable `magnitud_cat` fue definida como factor ordenado en `Codigo.R`, por lo que los graficos respetan el orden conceptual de severidad.
